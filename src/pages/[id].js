...
export default function Home() {
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        const data = await fetch('https://api-untukmu.septianfauzi.com/api/undangan/' + id);
        const json = await data.json();
        if (json.data.length === 0) {
          router.push("error/404")
        } else {
          setName(json.data[0].Nama)
        }
      }
      fetchData();
    }
  ...
  return (
    <>
      ...
      <div>
        {isLandingPage && <div id="opening" className="h-100 w-100 position-fixed d-flex justify-content-center align-items-center"
          style={{ zIndex: 20 }}>
          <div className="container mx-4">
            <div className="row d-flex flex-column justify-content-center align-items-center p-4 rounded">
              <div className='text-center my-2'>
                <Image src="https://septianfauzi.com/assets/undangan/images/NandaFauzi.gif" width={200} height={200} priority className="img-fluid rounded-circle" alt="No Image" />
              </div>
              <div className="great-vibes text-center my-2">
                <h1 className="text-main mb-1">Siti Nanda Nurdiani</h1>
                <h1 className="text-main mb-1">&amp;</h1>
                <h1 className="text-main mb-1">Septian Fauzi</h1>
              </div>
              <div className="text-center p-2 rounded">
                <p className="mb-1">Kepada Bapak/Ibu/Saudara/i</p>
                <p className="my-0 fw-bold">{name}</p>
              </div>
              <div className="text-center my-2">
                <p className="m-0 small">Kami mengundang Bapak/Ibu/Saudara/i untuk hadir
                  dalam acara pernikahan kami</p>
              </div>
              ...
              <div className="text-center">
                <p style={{ fontSize: 10 }} className="m-0">Mohon maaf apabila ada kesalahan nama dan gelar</p>
              </div>
            </div>
          </div>
        </div>}
